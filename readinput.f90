...
! broadcast to other process
	...
	if(myid==0) then
		...
		call MPI_PACK(nthreads,2,MPI_integer4,packbuf,packsize,position1,MPI_COMM_WORLD,ierr)
	...
	if(myid/=0) then
		...
		call MPI_UNPACK(packbuf,packsize,position1,nthreads,2,MPI_integer4,MPI_COMM_WORLD,ierr)
		if(mode=='r') then
