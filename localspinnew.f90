...
! the algorithm follows I.Mayer CPL 478(2009) 323-326
    ...
    real(kind=r8),allocatable :: localspin0(:,:,:),spinspincorrelation0(:,:,:,:,:)
...
! <R|<L|CLR OP CL'R'|L'>|R'>
    ...
    integer :: i,j,k,istate,jstate
    integer :: nmid
    integer :: error,ierr,info
    integer :: status(MPI_STATUS_SIZE) ! MPI flag
    real(kind=r8) :: Spratio
    real(kind=r8)   :: ilocalspin(nstate),ispinspincorrelation(2,nstate,nstate)
    ...
    do j=i+1,orbend,1
        if(myid==orbid3(i,j,1)) then
            ...
            !   ! ai^+down*aiup*aj^+up*aj^down  and  1/4*(niup-nidown)(njup-njdown) calculate respectively
                    call SpMMtoSp('N','N',4*subM,4*subM,4*subM,4*subM,4*subM,&
                        operamatbig3(:,operaindex3-2+k),bigcolindex3(:,operaindex3-2+k),bigrowindex3(:,operaindex3-2+k), &
                        coeffIF(:,istate),coeffIFcolindex(:,istate),coeffIFrowindexdummy(:,istate),&
                        midmat,midcolindex,midrowindex,nmid)
            ...
            call MPI_SEND(ispinspincorrelation,2*nstate*nstate,mpi_real8,0,orbid3(i,j,2),MPI_COMM_WORLD,ierr)
        
        else if(myid==0) then
            call MPI_RECV(ispinspincorrelation,2*nstate*nstate,mpi_real8,orbid3(i,j,1),orbid3(i,j,2),MPI_COMM_WORLD,status,ierr)
            do istate=1,nstate,1
            do jstate=1,nstate,1
                if(istate==jstate) then
                    localspin0(i,j,jstate)=ispinspincorrelation(1,istate,istate)+0.25*ispinspincorrelation(2,istate,istate)
                    localspin0(j,i,jstate)=localspin0(i,j,jstate)
                end if
                ! <jstate|ai^+aj|istate>
                if(domain=='L') then
                    ! i<j
                    spinspincorrelation0(i,j,1:2,jstate,istate)=ispinspincorrelation(1:2,jstate,istate)
                    spinspincorrelation0(j,i,1:2,jstate,istate)=ispinspincorrelation(1:2,istate,jstate)
                else if(domain=='R') then
                    ! i<j
                    spinspincorrelation0(i,j,1:2,jstate,istate)=ispinspincorrelation(1:2,istate,jstate)
                    spinspincorrelation0(j,i,1:2,jstate,istate)=ispinspincorrelation(1:2,jstate,istate)
                end if
            end do
            end do
        end if
...
! i<=nleft+1,j>=norbs-nright 
    ...
    real(kind=r8) :: ispinspincorrelation(2,nstate,nstate)
    integer :: i,j,k,l,m,istate,jstate
    ...
    midnelement=CEILING(DBLE(16*subM*subM)/pppVmidratio)
    ...
    do i=norbs,norbs-nright,-1
        ...
        do l=1,nleft+1,1
            if(myid==orbid3(l,l,1)) then
                ...
                call MPI_RECV(ispinspincorrelation,2*nstate*nstate,mpi_real8,orbid3(l,l,1),orbid3(l,l,2),MPI_COMM_WORLD,status,ierr)
                do istate=1,nstate,1
                do jstate=1,nstate,1
                    if(istate==jstate) then
                        localspin0(i,l,istate)=ispinspincorrelation(1,istate,istate)+0.25D0*ispinspincorrelation(2,istate,istate)
                        localspin0(l,i,istate)=localspin0(i,l,istate)
                    end if
                    !<jstate|al^+ai|istate>
                    spinspincorrelation0(l,i,1:2,jstate,istate)=ispinspincorrelation(1:2,jstate,istate)
                    spinspincorrelation0(i,l,1:2,jstate,istate)=ispinspincorrelation(1:2,istate,jstate)
                end do
                end do
            end if
        end do
