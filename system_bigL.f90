...
!     L sigmaL interaction operator contribute
		...
		do i=1,nleft,1
			...
			do j=1,2,1
				...
				call directproduct(operabuffer(1:Lrealdim,1:Lrealdim,j),Lrealdim,onesitemat(:,:,3+j),4,Hbuffer(1:4*Lrealdim,1:4*Lrealdim),phase(1:4*Lrealdim,1:4*Lrealdim))
				Hbig(1:4*Lrealdim,1:4*Lrealdim,1)=Hbig(1:4*Lrealdim,1:4*Lrealdim,1)+Hbuffer(1:4*Lrealdim,1:4*Lrealdim)*t(i,nleft+1)
			end do
			end if
			!     ppp term
				Hbuffer=0.0D0
				phase=1
				call directproduct(operabuffer(1:Lrealdim,1:Lrealdim,3),Lrealdim,onesitemat(:,:,3),4,Hbuffer(1:4*Lrealdim,1:4*Lrealdim))
